...
Not the same thing as :class:`router.embeddings.encoder.TextEncoder`, and kept
separate on purpose. That one is config-driven, serves the NIRT pathways, and
persists an ``EmbeddingStore`` keyed by query id. This one is keyed by *text
content*, applies the instruction prefix an ``intfloat/e5`` model was trained
with -- omitting it cost e5-large 3.25 points here, a real bug -- and exists so
the trained heads can be served the exact vectors they were fitted on.

It also keeps `decompose` dependency-free: importing the router's encoder from
here would need an import-linter exception, and the heads need this behaviour
