...
_MULTIPART_MARKERS = (
    " and then ", " after that ", " followed by ", "step by step", "first,", "1.", "2.",
    "; then", " as well as ",
)
...
class HeuristicTriage:
    ...
    def __call__(self, prompt: str, *, model_id: str, predicted_quality: float,
        ...
        weak = predicted_quality < self.quality_threshold

        if multipart and weak:
            reason = (f"multi-part prompt and best predicted quality "
                      f"{predicted_quality:.2f} < {self.quality_threshold:.2f}")
            mode = "orchestrate"
        elif multipart:
            reason = "prompt looks multi-part (decompose so each part routes independently)"
            mode = "orchestrate"
        elif weak:
            reason = (f"best candidate {model_id} predicted quality {predicted_quality:.2f} "
                      f"< {self.quality_threshold:.2f}; decompose to raise it")
            mode = "orchestrate"
        else:
            reason = (f"{model_id} predicted quality {predicted_quality:.2f} "
                      f">= {self.quality_threshold:.2f}; answer directly")
            mode = "single"
        return TriageDecision(mode, model_id, float(predicted_quality), reason, scores or {})


class LLMTriage:
    """Ask a LangChain chat model for a yes/no on decomposition; fall back to the
    heuristic on any parsing / call failure. The router's scores are given to the
    model as context so its judgement is grounded in the same difficulty signal."""

    _PROMPT = (
        "You triage user requests for an LLM router.\n"
        "Best available model: {model_id} (predicted quality {q:.2f} on a 0-1 scale).\n"
    ...
    def __call__(self, prompt: str, *, model_id: str, predicted_quality: float,
                 scores: Optional[dict] = None) -> TriageDecision:
        try:
            top = dict(sorted((scores or {}).items(), key=lambda kv: -kv[1])[:5])
            msg = self._chat.invoke(self._PROMPT.format(
                model_id=model_id, q=predicted_quality,
                scores=", ".join(f"{k}={v:.2f}" for k, v in top.items()) or "n/a",
                prompt=prompt[:2000],
            ))
            verdict = str(getattr(msg, "content", msg)).strip().upper()
        ...
        return self._fallback(prompt, model_id=model_id,
                              predicted_quality=predicted_quality, scores=scores)


def best_from_result(routing_result) -> tuple[str, float, dict]:
    """``(model_id, predicted_quality, {model_id: quality})`` for a 1-row
    :class:`~router.routing.base.RoutingResult`."""
    row = np.asarray(routing_result.scores)[0]
    scores = dict(zip(routing_result.model_ids, (float(x) for x in row)))
    mid = routing_result.selected_model_ids[0]
    return mid, float(routing_result.selected_scores[0]), scores
