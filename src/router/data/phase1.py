...
A Phase 1 training script imports **only this module**. It never touches
RouterBench pickles, Arena battle rows, lm-harness logs, or the canonical column
layout directly.
...
* **No guessing parameter.** Multiple-choice observations carry a data-level
  ...
  is also exposed for reference but Phase 1 is not expected to fit a ``c_q``.
...
class Phase1Data:
    ...
    def _default_pathway(self) -> str:
        from ..embeddings import available_pathways
    ...
    def profile_embeddings(self, pathway: Optional[str] = None):
        ...
        This is the vector Phase 1 uses to initialise ``theta_m`` (and to place
    ...
    def nirt_dataset(self, split: Optional[str] = "train", pathway: str = "irt",
                     query_pathway: Optional[str] = None, **kw):
        ...
        from .nirt import NIRTDataset

        return NIRTDataset.from_config(self.cfg, split=split, pathway=pathway,
                                       query_pathway=query_pathway, data=self, **kw)
...
def load_phase1(cfg: Config) -> Phase1Data:
    ...
    return Phase1Data(cfg, responses, queries, models, splits, profiles)
