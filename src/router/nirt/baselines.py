...
def fit_classical_irt(
    ...
    *,
    ...
    if protocol == "cell":
        ...
        fit_df = _matrix(fit_sp, "target").reindex(columns=m_ids)
        eval_df = _matrix(eval_sp, "target").reindex(columns=m_ids)
...
# --------------------------------------------------------------------------- #
# in-house router baselines (KNN = what RouterBench ships; MLP = IRT-structure #
# ablation -- "NIRT minus the bilinear form")                                  #
# --------------------------------------------------------------------------- #
...
def knn_router_matrix(
    ...
    *,
    ...
    C_imp = np.where(np.isnan(C), col_mean, C)
