"""Evaluate a trained NIRT / IRT-Router model.

    from router.nirt.train import load_run
    from router.nirt.evaluate import evaluate_split, cold_start_eval
...
def predict_matrix(model, model_index: dict, data, split: str, pathway: str = "irt",
                   query_pathway: Optional[str] = None) -> pd.DataFrame:
...
def evaluate_split(
    ...
    *,
    ...
    if data is None:
        ...
        data = load_phase1(phase0_cfg or load_config())
